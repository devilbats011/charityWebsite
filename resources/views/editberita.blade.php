        ...
        <div>
          <img src="https://www.striker.id/wp-content/uploads/2019/01/73766.jpg" alt="skopped!"> 
            <section class="wrapper">
             ...
             <form id="edit_form" enctype="multipart/form-data" action="{{route('testpost')}}" method="post" style="color:black;padding:20px 0px" >
               @csrf
               <input type="hidden" value="{{$beritas->id}}" name="id">
               <input type="file" onchange="setFileForm(this.files)" name="gambar-1" style="color:white" >
               <input id="tajuk" name="tajuk" value="{{ $beritas->tajuk }}" required ><br>
               <textarea name="text-1" required >   {{ $beritas->text1 }} </textarea>
               <section id="appendMe">
